...
iptables 每flush POSTROUTING 每table nat
iptables 每flush FORWARD
...
iptables -t nat -A POSTROUTING -s 172.16.36.0/24 -j SNAT 每to-source `ifconfig | grep ＆inet addr:＊| grep -v ＊127.0.0.1∩ | cut -d: -f2 | awk ＆NR==1 { print $1}＊`
iptables -A FORWARD -p tcp 每syn -s 172.16.36.0/24 -j TCPMSS 每set-mss 1356
